...
export default function PrintDeliveryPanel({ production, mockupUrl, designId, size, color, title }) {
  ...
  const standard = getPrintStandard(production?.product_type);
  ...
  return <section className="overflow-hidden rounded-2xl border bg-card text-card-foreground shadow-sm">
    <div className="grid md:grid-cols-[240px_1fr]">
      ...
      <div className="space-y-5 p-5 sm:p-6">
        ...
        <div className="grid gap-2 sm:grid-cols-2">{sides.map(side => <PrintReadyDownloadButton key={side} sourceUrl={production[side].print_ready_url || production[side].file_url} fileName={`estampa-${designId || 'design'}-${side}`} sideLabel={side === 'front' ? 'frontal em alta resolução (PNG)' : 'traseira em alta resolução (PNG)'} productType={production.product_type} />)}</div>
        <p className="text-xs text-muted-foreground">PNG isolado com transparência em {standard.widthCm} × {standard.heightCm} cm, {standard.widthPx} × {standard.heightPx} px a {standard.dpi} DPI, sem camiseta ou fundo.</p>
      </div>
    </div>
  </section>;
}