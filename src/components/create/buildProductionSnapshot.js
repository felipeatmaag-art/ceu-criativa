...
export default async function buildProductionSnapshot(options) {
  ...
  const production = { version: 1, placement_version: 1, catalog_product_id: product?.id || '', product_type: productType, color, size, approved_at: new Date().toISOString(), print_area: { ...PRINT_AREA, coordinate_space: 1000, calibrated: false }, technical_review: 'pending' };
