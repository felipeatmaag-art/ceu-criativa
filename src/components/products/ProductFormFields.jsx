...
export default function ProductFormFields({ form, onChange }) {
  ...
  return (
    <div className="grid min-w-0 gap-4 sm:grid-cols-2 [&>div]:min-w-0">
      ...
      <div><Label>Tipo</Label><select {...field('type')} className="mt-1 h-10 w-full rounded-md border bg-background px-3">{types.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
