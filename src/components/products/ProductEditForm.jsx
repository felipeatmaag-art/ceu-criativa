...
export default function ProductEditForm({ product, onUpdated, onCancel }) {
  ...
  return <form onSubmit={submit} className="min-w-0 max-w-full space-y-6 text-ceu-navy"><ProductFormFields form={form} onChange={(name, value) => setForm((current) => ({ ...current, [name]: value }))} />{error && <p className="text-sm text-destructive" role="alert">{error}</p>}<div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button type="button" variant="outline" disabled={saving} onClick={onCancel} className="rounded-full">Cancelar</Button><Button disabled={saving} className="rounded-full bg-ceu-navy px-6 text-ceu-cloud">{saving ? 'Salvando...' : 'Salvar alterações'}</Button></div></form>;
}