...
export default function ProductAdmin() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-ceu-cloud px-4 py-12"><div className="mx-auto max-w-6xl"><Tabs defaultValue="catalog"><TabsList className="mb-8 rounded-full bg-card p-1 shadow-sm"><TabsTrigger value="catalog" className="rounded-full px-5">Catálogo</TabsTrigger><TabsTrigger value="finance" className="rounded-full px-5">Financeiro</TabsTrigger></TabsList><TabsContent value="catalog"><div className="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold uppercase tracking-widest text-ceu-navy">Catálogo SEL</p><h1 className="mt-2 text-4xl font-bold text-ceu-navy">Gerenciar catálogo</h1><p className="mt-2 max-w-3xl text-muted-foreground">Mantenha material e modelagem organizados, envie a foto de cada modelo e configure as cores que serão disponibilizadas no painel Criar.</p></div><ProductCatalogDialog onCreated={createProduct}/></div>{loading ? <p className="text-muted-foreground">Carregando produtos...</p> : error ? <div className="rounded-3xl border bg-card p-8 text-center"><p className="text-destructive">{error}</p><Button type="button" variant="outline" onClick={loadProducts} className="mt-4 rounded-full">Tentar novamente</Button></div> : <ProductCatalogList products={products} onUpdated={updateProduct} onDeleted={deleteProduct}/>}</TabsContent><TabsContent value="finance"><AdminFinancialDashboard/></TabsContent></Tabs></div></div>;
}