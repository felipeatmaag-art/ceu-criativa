...
export function buildPodQuote(product, design, quantity = 1) {
  ...
  const baseCost = money(product?.base_cost ?? product?.base_price);
  ...
  const platformMargin = money(Math.max(0, designPrice - artistCommission));
  const unitPrice = money(baseCost + artistCommission + platformMargin);
  return {
    quantity: units,
    base_cost: baseCost,
