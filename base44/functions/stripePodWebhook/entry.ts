...
export default async function(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (['payment_intent.amount_capturable_updated', 'payment_intent.succeeded', 'payment_intent.canceled'].includes(event.type) && event.data.object.metadata?.base44_app_id === secrets.get('BASE44_APP_ID') && event.data.object.metadata?.order_id) {
      const orders = await base44.asServiceRole.entities.Order.filter({ id: event.data.object.metadata.order_id }, '-created_date', 1);
      if (orders[0]?.payment_intent_id === event.data.object.id) await settlePodPayment(base44, event.data.object);
    }
